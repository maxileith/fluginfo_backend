...
class StatusExact:
    ...
    def get(flight_number: str, date: str) -> dict:
        ...
        status = [
            a for a in status if a['flightNumber'] == flight_number]
        try:
            status[0]['departure']['airport'] = Airport.details(
                status[0]['departure']['airport']['iata'])
            status[0]['arrival']['airport'] = Airport.details(
                status[0]['arrival']['airport']['iata'])
            return status[0]
        except IndexError:
...
class StatusSearch:
    ...
    def get(departure_iata: str, arrival_iata: str, date: str) -> list:
        ...
        status = response.result['data']

        return StatusSearch.__simplify_status(status)

    @staticmethod
    def __simplify_status(status: list) -> list:
        simplified_status = list()
        for a in status:
            ...
            # availability searches
            try:
                simplified_status[-1]['carrier'] = bookshelf.get(
